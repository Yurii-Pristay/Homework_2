...
import { aRoute, dbObject } from "../interfaces"

module.exports = class CrudForARoute {
readonly dbArray: Array<dbObject>;
readonly  promise: Promise<string>;
    ...
    POST(req, res) {
        ...
        return this.promise.then(users => {
            ...
            const objToCreate: aRoute = {id: newId, a};
    ...
    PUT(req, res) {
        ...
        return this.promise.then(users => {
            ...
            const { a } = JSON.parse(users);
            const findObject:dbObject = this.dbArray.find((user) =>  user.a);
            if(!findObject){
